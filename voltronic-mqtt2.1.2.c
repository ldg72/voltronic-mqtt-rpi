...
unsigned short crc16(const unsigned char *buf, size_t len) {
    ...
    while (len--) { crc ^= *buf++ << 8; for (int i = 0; i < 8; i++) crc = crc & 0x8000 ? (crc << 1) ^ 0x1021 : crc << 1; }
    return crc;
}
...
void publish_metric(MQTTClient client, const char* base_topic, const char* sub_topic, const char* metric_name, const char* value_str) {
    ...
    char full_topic[256];
    sprintf(full_topic, "%s/%s/%s", base_topic, sub_topic, metric_name);
...
int main(int argc, char* argv[]) {
    ...
    sprintf(mqtt_address, "tcp://%s:1883", mqtt_host);
    ...
    while (1) {
        ...
        PIRIData piri_data = {0};
        PIGS2Data pigs2_data = {0};
        char qmod_char = ' ';
        char qid_response[32] = "N/A";
        PIGSData pigs_data = {0};
